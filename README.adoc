...
Dessert provides an API to express such block diagrams in Java code.

A block is an arbitrary part of a software-system. Dessert provides
the `Slice` interface to express such blocks. The arrows are
dependencies between the blocks. Dessert's assertion API expresses
such dependencies.
